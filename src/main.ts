...
export default class WordpressPlugin extends Plugin
{
    ...
    async onload()
    {
        ...
        this.addCommand( {
            id : "defaultPublish", name : this.#i18n.t( "command_publishWithDefault" ), editorCallback : () =>
            {
                const defaultProfile = this.#settings?.profiles.find( it => it.isDefault );
                if ( defaultProfile )
                {
                    const params : WordPressPostParams = {
                        status : this.#settings?.defaultPostStatus ?? PostStatus.Draft, commentStatus : this.#settings?.defaultCommentStatus ?? CommentStatus.Open, categories : defaultProfile.lastSelectedCategories ?? [ 1 ], postType : PostTypeConst.Post, tags : [], title : "", content : ""
                    };
        ...
        this.addCommand( {
            id : "publish", name : this.#i18n.t( "command_publish" ), editorCallback : () =>
            {
                ...
                if ( e.state )
                {
                    if ( e.error )
                    {
                        showError( this.i18n.t( "error_wpComAuthFailed", {
                            error : e.error, desc : e.error_description.replace( /\+/g, " " )
                        } ) );
                        AppState.events.trigger( EventType.OAUTH2_TOKEN_GOT, undefined );
                    }
                    else if ( e.code )
                    {
                        const token = await OAuth2Client.getWpOAuth2Client( this )
                        .getToken( {
                            code : e.code, redirectUri : WP_OAUTH2_REDIRECT_URI, codeVerifier : AppState.codeVerifier
                        } );
                        console.log( token );
