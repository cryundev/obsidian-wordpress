...
export class RestClient
{
    ...
    async httpGet( path : string, options? : {
    ...
    } ) : Promise<unknown>
    {
        ...
        const response = await requestUrl( {
            url : endpoint, method : "GET", headers : {
                "content-type" : "application/json", "user-agent" : "obsidian.md", ...opts.headers
            }
        } );
    ...
    async httpPost( path : string, body : SafeAny, options : {
        headers? : Record<string, string>; formItemNameMapper? : FormItemNameMapper;
    } ) : Promise<unknown>
    {
        let realPath = path;
        if ( realPath.startsWith( "/" ) )
        ...
        if ( body instanceof FormItems )
        {
            ...
            requestBody = await body.toArrayBuffer( {
                boundary, nameMapper : options.formItemNameMapper
            } );
        ...
        const response = await requestUrl( {
            url : endpoint, method : "POST", headers : {
                "user-agent" : "obsidian.md", ...predefinedHeaders, ...options.headers
            }, body : requestBody
        } );
