...
export class PassCrypto
{
    ...
    canUse() : boolean
    {
        return !isNil( crypto ) && !isNil( crypto.subtle ) && isFunction( crypto.getRandomValues ) && isFunction( crypto.subtle.generateKey ) && isFunction( crypto.subtle.encrypt ) && isFunction( crypto.subtle.decrypt ) && isFunction( crypto.subtle.importKey ) && isFunction( crypto.subtle.exportKey );
    }
    ...
    async encrypt( message : string ) : Promise<{ encrypted : string, key? : string, vector? : string }>
    {
        if ( this.canUse() )
        {
            ...
            return {
                key : JSON.stringify( exportedKey ), vector : this.bufferToBase64( vector ), encrypted : this.bufferToBase64( encrypted )
            };
        }
        else
        {
        ...
        if ( this.canUse() )
        {
            if ( key && vector )
            {
                ...
                const decrypted = await crypto.subtle.decrypt( {
                    name : AES_GCM, iv : this.base64ToBuffer( vector )
                }, thisKey, this.base64ToBuffer( encrypted ) );
