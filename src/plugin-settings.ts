...
{
    XML_RPC = "xml-rpc", RestAPI_miniOrange = "miniOrange", RestApi_ApplicationPasswords = "application-passwords", RestApi_WpComOAuth2 = "WpComOAuth2"
}
...
export const DEFAULT_SETTINGS : WordpressPluginSettings = {
    lang : "auto", profiles : [], showRibbonIcon : false, defaultPostStatus : PostStatus.Draft, defaultCommentStatus : CommentStatus.Open, rememberLastSelectedCategories : true, showWordPressEditConfirm : false, mathJaxOutputType : MathJaxOutputType.SVG, commentConvertMode : CommentConvertMode.Ignore, enableHtml : false, replaceMediaLinks : true
};

export async function upgradeSettings(
        existingSettings : SafeAny,
        to : SettingsVersion ) : Promise<{ needUpgrade : boolean, settings : WordpressPluginSettings }>
{
    ...
    if ( isUndefined( existingSettings.version ) )
    {
        ...
        if ( to === SettingsVersion.V2 )
        {
            const newSettings : WordpressPluginSettings = Object.assign( {}, DEFAULT_SETTINGS, {
                version : SettingsVersion.V2, lang : existingSettings.lang, showRibbonIcon : existingSettings.showRibbonIcon, defaultPostStatus : existingSettings.defaultPostStatus, defaultCommentStatus : existingSettings.defaultCommentStatus, defaultPostType : "post", rememberLastSelectedCategories : existingSettings.rememberLastSelectedCategories, showWordPressEditConfirm : existingSettings.showWordPressEditConfirm, mathJaxOutputType : existingSettings.mathJaxOutputType, commentConvertMode : existingSettings.commentConvertMode
            } );
            if ( existingSettings.endpoint )
            {
                ...
                const profile = {
                    name : WP_DEFAULT_PROFILE_NAME, apiType : apiType, endpoint : endpoint, xmlRpcPath : xmlRpcPath, saveUsername : !isNil( username ), savePassword : !isNil( password ), isDefault : true, lastSelectedCategories : lastSelectedCategories, username : username, encryptedPassword : encryptedPassword
                };
            ...
            return {
                needUpgrade : true, settings : newSettings
            };
        }
    }
    return {
        needUpgrade : false, settings : existingSettings
    };
}
