...
export class WpPublishModal extends AbstractModal
{
    ...
    constructor(
            ...
            private readonly categories : {
                items : Term[], selected : number[]
            },
            private readonly postTypes : {
                items : PostType[], selected : PostType
            },
            private readonly onSubmit : (
                    params : WordPressPostParams,
                    updateMatterData : ( matter : MatterData ) => void ) => void,
    ...
    onOpen()
    {
        const params : WordPressPostParams = {
            status : this.plugin.settings.defaultPostStatus, commentStatus : this.plugin.settings.defaultCommentStatus, postType : this.postTypes.selected, categories : this.categories.selected, tags : [], title : "", content : ""
        };
    ...
    private display( params : WordPressPostParams ) : void
    {
        ...
        if ( params.status === PostStatus.Future )
        {
            ...
            .addText( text =>
            {
                ...
                const dateBlocks = {
                    yyyy : {
                        mask : IMask.MaskedRange, from : 1970, to : 9999
                    }, MM : {
                        mask : IMask.MaskedRange, from : 1, to : 12
                    }, dd : {
                        mask : IMask.MaskedRange, from : 1, to : 31
                    }
                };
                const dateMask = {
                    mask : Date, lazy : false, overwrite : true
                };
                if ( this.dateInputMask )
                {
                    this.dateInputMask.destroy();
                ...
                this.dateInputMask = IMask( text.inputEl, [ {
                    ...dateMask, pattern : dateFormat, blocks : dateBlocks, format : ( date : SafeAny ) => format( date, dateFormat ), parse : ( str : string ) => parse( str, dateFormat, new Date() )
                }, {
                    ...dateMask, pattern : dateTimeFormat, blocks : {
                        ...dateBlocks, HH : {
                    ...
                    }, format : ( date : SafeAny ) => format( date, dateTimeFormat ), parse : ( str : string ) => parse( str, dateTimeFormat, new Date() )
                } ] );
        ...
        .onClick( () =>
        {
            if ( this.matterData.postType && this.matterData.postType !== PostTypeConst.Post && ( this.matterData.tags || this.matterData.categories ) )
            {
                openConfirmModal( {
