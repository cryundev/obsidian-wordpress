...
{
    return new URLSearchParams( Object.fromEntries( Object.entries( params )
    .filter( ( [ k, v ] ) => v !== undefined ) ) as Record<string, string> ).toString();
}
...
export function doClientPublish(
        ...
        profile : WpProfile,
        defaultPostParams? : WordPressPostParams ) : void;
export function doClientPublish(
        plugin : WordpressPlugin,
        profileName : string,
        defaultPostParams? : WordPressPostParams ) : void;
        ...
        defaultPostParams? : WordPressPostParams ) : void
{
    ...
    if ( profile )
    {
        ...
        if ( client )
        {
            client.publishPost( defaultPostParams )
            .then();
        }
    }
    else
...
export function showError<T>( error : unknown ) : WordPressClientResult<T>
{
    ...
    return {
        code : WordPressClientReturnCode.Error as const, error : {
            code : WordPressClientReturnCode.Error, message : errorMessage
        }
    };
}
