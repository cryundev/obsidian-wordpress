import {
    WordPressAuthParams, WordPressClientResult, WordPressClientReturnCode, WordPressMediaUploadResult, WordPressPostParams, WordPressPublishResult
} from "./wp-client";
...
export class WpRestClient extends AbstractWordPressClient
{
            ...
            certificate : WordPressAuthParams ) : Promise<WordPressClientResult<WordPressPublishResult>>
    {
        ...
        const resp : SafeAny = await this.client.httpPost( url, {
            title, content, status : postParams.status, comment_status : postParams.commentStatus, categories : postParams.categories, tags : postParams.tags ?? [], ...extra
        }, {
        ...
        try
        {
            ...
            return {
                code : WordPressClientReturnCode.OK, data : result, response : resp
            };
        }
        catch ( e )
        {
            return {
                code : WordPressClientReturnCode.Error, error : {
                    code : WordPressClientReturnCode.ServerInternalError, message : this.plugin.i18n.t( "error_cannotParseResponse" )
                }, response : resp
            };
        }
    }
    ...
    {
        const data = await this.client.httpGet( getUrl( this.context.endpoints?.getCategories, "wp-json/wp/v2/categories?per_page=100" ), {
            headers : this.context.getHeaders( certificate )
        } );
    ...
    {
        const data : SafeAny = await this.client.httpGet( getUrl( this.context.endpoints?.getPostTypes, "wp-json/wp/v2/types" ), {
            headers : this.context.getHeaders( certificate )
        } );
    ...
    async validateUser( certificate : WordPressAuthParams ) : Promise<WordPressClientResult<boolean>>
    {
        try
        {
            const data = await this.client.httpGet( getUrl( this.context.endpoints?.validateUser, `wp-json/wp/v2/users/me` ), {
                headers : this.context.getHeaders( certificate )
            } );
            return {
                code : WordPressClientReturnCode.OK, data : !!data, response : data
            };
        }
        catch ( error )
        {
            return {
                code : WordPressClientReturnCode.Error, error : {
                    code : WordPressClientReturnCode.Error, message : this.plugin.i18n.t( "error_invalidUser" )
                }, response : error
            };
        }
    }
    ...
    {
        const termResp : SafeAny = await this.client.httpGet( getUrl( this.context.endpoints?.getTag, "wp-json/wp/v2/tags?number=1&search=<%= name %>", {
            name
        } ) );
        const exists = this.context.responseParser.toTerms( termResp );
        if ( exists.length === 0 )
        {
            const resp = await this.client.httpPost( getUrl( this.context.endpoints?.newTag, "wp-json/wp/v2/tags" ), {
                name
            }, {
                headers : this.context.getHeaders( certificate )
            } );
            ...
            certificate : WordPressAuthParams ) : Promise<WordPressClientResult<WordPressMediaUploadResult>>
    {
        try
        {
            ...
            const response : SafeAny = await this.client.httpPost( getUrl( this.context.endpoints?.uploadFile, "wp-json/wp/v2/media" ), formItems, {
                headers : {
            ...
            return {
                code : WordPressClientReturnCode.OK, data : result, response
            };
        }
        catch ( e : SafeAny )
        {
            ...
            return {
                code : WordPressClientReturnCode.Error, error : {
                    code : WordPressClientReturnCode.ServerInternalError, message : e.toString()
                }, response : undefined
            };
        }
    }
...
function getUrl(
        ...
        defaultValue : string,
        params? : { [ p : string ] : string | number } ) : string
{
...
interface WpRestClientContext
{
    ...
    responseParser : {
        toWordPressPublishResult : (
                postParams : WordPressPostParams,
                response : SafeAny ) => WordPressPublishResult; /**
         ...
         */
        toWordPressMediaUploadResult : ( response : SafeAny ) => WordPressMediaUploadResult; toTerms : ( response : SafeAny ) => Term[]; toTerm : ( response : SafeAny ) => Term; toPostTypes : ( response : SafeAny ) => PostType[];
    };
...
class WpRestClientCommonContext implements WpRestClientContext
{
    ...
    responseParser = {
        toWordPressPublishResult : ( postParams : WordPressPostParams, response : SafeAny ) : WordPressPublishResult =>
        {
            ...
            {
                return {
                    postId : postParams.postId ?? response.id, categories : postParams.categories ?? response.categories
                };
            }
            throw new Error( "xx" );
...
export class WpRestClientWpComOAuth2Context implements WpRestClientContext
{
    ...
    endpoints : WpRestEndpoint = {
        base : "https://public-api.wordpress.com", newPost : () => `/rest/v1.1/sites/${ this.site }/posts/new`, editPost : () => `/rest/v1.1/sites/${ this.site }/posts/<%= postId %>`, getCategories : () => `/rest/v1.1/sites/${ this.site }/categories`, newTag : () => `/rest/v1.1/sites/${ this.site }/tags/new`, getTag : () => `/rest/v1.1/sites/${ this.site }/tags?number=1&search=<%= name %>`, validateUser : () => `/rest/v1.1/sites/${ this.site }/posts?number=1`, uploadFile : () => `/rest/v1.1/sites/${ this.site }/media/new`, getPostTypes : () => `/rest/v1.1/sites/${ this.site }/post-types`
    };
    ...
    responseParser = {
        toWordPressPublishResult : ( postParams : WordPressPostParams, response : SafeAny ) : WordPressPublishResult =>
        {
            ...
            {
                return {
                    postId : postParams.postId ?? response.ID, categories : postParams.categories ?? Object.values( response.categories )
                    .map( ( cat : SafeAny ) => cat.ID )
                };
            }
            throw new Error( "xx" );
        }, toWordPressMediaUploadResult : ( response : SafeAny ) : WordPressMediaUploadResult =>
