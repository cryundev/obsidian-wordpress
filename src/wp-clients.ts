...
import {
    WpRestClient, WpRestClientAppPasswordContext, WpRestClientMiniOrangeContext, WpRestClientWpComOAuth2Context
} from "./wp-rest-client";
...
export function getWordPressClient( plugin : WordpressPlugin, profile : WpProfile ) : WordPressClient | null
{
    ...
    case ApiType.RestApi_WpComOAuth2:
        ...
        {
            client = new WpRestClient( plugin, profile, new WpRestClientWpComOAuth2Context( profile.wpComOAuth2Token.blogId, profile.wpComOAuth2Token.accessToken ) );
        }
        else
        {
